...
- Conference authors list
- Scholar and author details
...
class Paper(BaseModel):
    ...
    aminer_paper_id: Optional[str] = None
...
class AuthorDetail(BaseModel):
    ...
    # From enriched data (if available)
    homepage: Optional[str] = None
    google_scholar: Optional[str] = None
    dblp: Optional[str] = None
    linkedin: Optional[str] = None
...
@app.get("/api/conferences/{conference_id}/authors", response_model=list[AuthorBasic])
def get_conference_authors(
    conference_id: str,
    limit: Optional[int] = Query(None, description="Limit number of authors returned"),
    offset: Optional[int] = Query(0, description="Offset for pagination"),
):
    """
    Get all paper authors for a specific conference.
    Returns list of authors with their papers and basic statistics.
    ...
    talents = authors_data.get("talents", [])

    # Filter to only include authors with AMiner data file
    talents_with_aminer = []
    for t in talents:
        aminer_id = t.get("aminer_id")
        if aminer_id:
            aminer_path = settings.aminer_scholars_dir / f"{aminer_id}.json"
            if aminer_path.exists():
                talents_with_aminer.append(t)

    # Apply pagination
    if limit is not None:
        talents_paginated = talents_with_aminer[offset:offset + limit]
    elif offset > 0:
        talents_paginated = talents_with_aminer[offset:]
    else:
        talents_paginated = talents_with_aminer

    # Build author list
    authors = []
    for talent in talents_paginated:
        aminer_id = talent.get("aminer_id")
        photo_url = get_scholar_photo(aminer_id)

        # Convert papers
        papers = [Paper(**paper) for paper in talent.get("papers", [])]

        # Convert statistics
        stats_data = talent.get("statistics", {})
        statistics = AuthorStatistics(
            total_papers=stats_data.get("total_papers", 0),
            tracks=stats_data.get("tracks", []),
            collaborators_count=stats_data.get("collaborators_count", 0)
        )

        authors.append(AuthorBasic(
            name=talent.get("name", "Unknown"),
            roles=talent.get("roles", []),
            papers=papers,
            statistics=statistics,
            collaborators=talent.get("collaborators", []),
            description=talent.get("description"),
            aminer_id=aminer_id,
            photo_url=photo_url,
        ))
...
@app.get("/api/conferences/{conference_id}/authors/search", response_model=list[AuthorBasic])
def search_authors(
    conference_id: str,
    name: Optional[str] = Query(None, description="Author name to search (case-insensitive, partial match)"),
    track: Optional[str] = Query(None, description="Filter by track name"),
    min_papers: Optional[int] = Query(None, description="Minimum number of papers"),
    aminer_id: Optional[str] = Query(None, description="AMiner ID to search (exact match)"),
    limit: Optional[int] = Query(None, description="Limit number of authors returned"),
    offset: Optional[int] = Query(0, description="Offset for pagination"),
):
    """
    Search and filter authors by various criteria.
    ...
    filtered_talents = []
    for talent in talents:
        # Only include authors with AMiner data file
        aminer_id = talent.get("aminer_id")
        if not aminer_id:
            continue
        aminer_path = settings.aminer_scholars_dir / f"{aminer_id}.json"
        ...
        if name and name.lower() not in talent.get("name", "").lower():
            continue

        # AMiner ID filter (exact match)
        if aminer_id and talent.get("aminer_id") != aminer_id:
            continue
        ...
        if track:
            stats_data = talent.get("statistics", {})
            talent_tracks = stats_data.get("tracks", [])
            if track not in talent_tracks:
                continue
    ...
    # Apply pagination
    if limit is not None:
        filtered_talents = filtered_talents[offset:offset + limit]
    elif offset > 0:
        filtered_talents = filtered_talents[offset:]

    # Build author list
    authors = []
    for talent in filtered_talents:
        aminer_id_val = talent.get("aminer_id")
        photo_url = get_scholar_photo(aminer_id_val)
...
@app.get("/api/conferences/{conference_id}/authors/count")
def get_authors_count(conference_id: str):
    """
    Get the count of authors with AMiner data file for a specific conference.
    """
    conference_dir = settings.data_dir / conference_id
    authors_path = conference_dir / "authors.json"
    ...
    return {"count": count_with_aminer}


def build_author_detail(talent: dict, aminer_id: Optional[str]) -> AuthorDetail:
    """Build detailed author information from multiple sources."""
    photo_url = get_scholar_photo(aminer_id)
...
@app.get("/api/conferences/{conference_id}/authors/{author_name}", response_model=AuthorDetail)
def get_author_detail(conference_id: str, author_name: str):
    """
    Get detailed information for a specific author.
    Includes papers, statistics, AMiner data, and enriched data if available.
    """
    conference_dir = settings.data_dir / conference_id
    authors_path = conference_dir / "authors.json"
    ...
    for talent in talents:
        if talent.get("name", "").lower() == author_name.lower():
            matching_talent = talent
            break

    if not matching_talent:
        raise HTTPException(status_code=404, detail=f"Author not found: {author_name}")

    # Build detailed response
    author_aminer_id = matching_talent.get("aminer_id")
    detail = build_author_detail(matching_talent, author_aminer_id)
